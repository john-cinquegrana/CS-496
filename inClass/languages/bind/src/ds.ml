...
let rec in_env : string -> exp_val ea_result = fun id env ->
  match env with
...
let rec un_bound : string -> env ea_result = fun id env ->
  match env with
  | EmptyEnv -> Ok EmptyEnv
  | ExtendEnv(v, ev, tail) ->
  if id=v
  then Ok tail
  else ( un_bound id >>+ extend_env v ev ) env
