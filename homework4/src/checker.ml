...
declaration")
  ...
  | Unpair(id1,id2,e1,e2) ->
    type_of_expr e1 >>=
    pair_of_pairType "pair: " >>= fun (t1, t2) ->
    extend_tenv id1 t1 >>+ 
    extend_tenv id2 t2 >>+
    type_of_expr e2 
  ...
  | DeRef(e) ->
    type_of_expr e >>=
    arg_of_refType "ref: " (*TODO*)
  | SetRef(e1,e2) ->
    type_of_expr e1 >>=
    arg_of_refType "ref: " >>= fun _ ->
  ...
  | Cons(h, t) ->
    ...
    type_of_expr t >>=
    arg_of_listType "list: " >>= fun t2 -> begin
    if (t1 = t2) then
      return @@ ListType(t1)
    else
      error "Types do not match" (*TODO*)
    end
  | Null(e) ->
     type_of_expr e >>=
     arg_of_listType "list: " >>= fun _ ->
     return BoolType
  | Hd(e) ->
    type_of_expr e >>=
  ...
  | GetData(t) ->
    ...
    return @@ t1
  | GetLST(t) ->
    type_of_expr t >>= fun tree ->
    arg_of_treeType "tree: " tree >>= fun _ ->
    return tree
  | GetRST(t) ->
    type_of_expr t >>= fun tree ->
    arg_of_treeType "tree: " tree  >>= fun _ ->
